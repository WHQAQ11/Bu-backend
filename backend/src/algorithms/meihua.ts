...
export function meihuaWordDivination(text: string): MeihuaGua {
  ...
  if (chars.length === 1) {
    ...
    // 两字：前字为上卦，后字为下卦
    upperNum = getStrokeCount(chars[0]) > 8 ?
      (getStrokeCount(chars[0]) % 8 || 8) : getStrokeCount(chars[0]);
    lowerNum = getStrokeCount(chars[1]) > 8 ?
      (getStrokeCount(chars[1]) % 8 || 8) : getStrokeCount(chars[1]);
  } else {
    // 多字：前半部分为上卦，后半部分为下卦
...
// 生成十翼解释
function generateShiyi(guaName: string): string {
  const shiyiMap: { [key: string]: string } = {
...
// 生成梅花易数分析
function generateMeihuaAnalysis(guaName: string, changingYao: number | undefined, method: string): string {
  ...
  // 根据卦象给出基本分析
  const guaAnalysis: { [key: string]: string } = {
