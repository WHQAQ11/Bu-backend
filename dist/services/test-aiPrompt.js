...
const liuyaoPrompt = aiPromptBuilder_1.AIPromptBuilder.buildDivinationPrompt(liuyaoContext, liuyaoOptions);
...
const meihuaPrompt = aiPromptBuilder_1.AIPromptBuilder.buildDivinationPrompt(meihuaContext, meihuaOptions);
console.log('提示词长度:', meihuaPrompt.length, '字符');
console.log('提示词预览:\n', meihuaPrompt.substring(0, 400) + '...\n');
console.log('\n3. 快速提示词测试:');
const quickPrompt = aiPromptBuilder_1.AIPromptBuilder.buildQuickPrompt(liuyaoContext);
console.log('快速提示词:\n', quickPrompt);
console.log('提示词长度:', quickPrompt.length, '字符\n');
console.log('4. 英文提示词测试:');
const englishPrompt = aiPromptBuilder_1.AIPromptBuilder.buildEnglishPrompt(liuyaoContext);
console.log('英文提示词长度:', englishPrompt.length, '字符');
console.log('英文提示词预览:\n', englishPrompt.substring(0, 600) + '...\n');
console.log('5. 情感化提示词测试:');
const emotionalPrompt = aiPromptBuilder_1.AIPromptBuilder.buildEmotionalPrompt(meihuaContext);
console.log('情感化提示词长度:', emotionalPrompt.length, '字符');
console.log('情感化提示词预览:\n', emotionalPrompt.substring(0, 500) + '...\n');
console.log('\n6. 不同关注领域测试:');
const focuses = ['career', 'relationship', 'health', 'wealth', 'general'];
focuses.forEach(focus => {
    ...
    const focusPrompt = aiPromptBuilder_1.AIPromptBuilder.buildDivinationPrompt(liuyaoContext, focusOptions);
    console.log(`${focus}关注点提示词长度:`, focusPrompt.length, '字符');
});
console.log('\n=== AI 提示词构造器测试完成 ===');
//# sourceMappingURL=test-aiPrompt.js.map