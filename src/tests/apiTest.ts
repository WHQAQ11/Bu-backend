...
describe('占卜系统API测试', () => {
  ...
  describe('占卜计算接口测试', () => {
    test('六爻占卜计算 - 基本功能', async () => {
      ...
      expect(originalGua.yaos.every((yao: any) => yao.value >= 6 && yao.value <= 9)).toBe(true);
    ...
    test('梅花易数占卜计算', async () => {
      ...
      expect(data.result).toHaveProperty('result'); // 梅花易数的嵌套结构
    ...
    test('不同占卜方法结果差异性', async () => {
      ...
      // 六爻占卜
      const liuyaoResponse = await request(app)
        .post('/api/divination/calculate')
        .set('Authorization', `Bearer ${authToken}`)
        .send({
          method: 'liuyao',
      ...
      // 梅花易数
      const meihuaResponse = await request(app)
        .post('/api/divination/calculate')
        .set('Authorization', `Bearer ${authToken}`)
        .send({
          method: 'meihua',
  ...
  describe('AI解读接口测试', () => {
    ...
    beforeAll(async () => {
      // 创建一个占卜记录用于AI解读测试
      const divinationResponse = await request(app)
        .post('/api/divination/calculate')
        .set('Authorization', `Bearer ${authToken}`)
        .send({
          method: 'liuyao',
